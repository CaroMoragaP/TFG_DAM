...
class BaseBookFields(BaseModel):
    ...
    @field_validator(*_OPTIONAL_BOOK_TEXT_FIELDS)
    @classmethod
    ...
    @classmethod
    def normalize_theme_values(cls, value: list[str] | None) -> list[str] | None:
        if value is None:
            return None
        return normalize_theme_list(value)


class BookCreate(BaseBookFields):
    library_id: int
    ...
    @classmethod
    def normalize_copy_locations(cls, value: str | None) -> str | None:
        return normalize_optional_text(value)
...
class BookMetadataUpdate(BaseBookFields):
    title: str | None = Field(default=None, min_length=1, max_length=255)
    authors: list[str] | None = None
    themes: list[str] | None = None
